...
func TestApplyDamageAndKill(t *testing.T) {
	...
	board.SetGameStart(time.Now().Add(-time.Minute))

	board.Apply(logparse.Event{
	...
	if p, _ := board.Player("1"); p.Damage != 100 {
		t.Errorf("attacker damage = %d, want 100", p.Damage)
	}
	if p, _ := board.Player("2"); p.DamageTaken != 100 {
	...
	if p, _ := board.Player("1"); p.Kills != 1 {
		t.Errorf("attacker kills = %d, want 1", p.Kills)
	}
	if p, _ := board.Player("2"); p.Deaths != 1 {
...
func TestSpectatorsNoStats(t *testing.T) {
	...
	if _, ok := board.Player("9"); ok {
		t.Errorf("spectator should not be tracked")
	}
	if _, ok := board.Player("1"); ok {
...
func TestMapChangeResets(t *testing.T) {
	...
	board.Apply(logparse.Event{Type: logparse.EventMapChange, Data: map[string]string{"map": "cp_process_f12"}})

	if _, ok := board.Player("1"); ok {
		t.Errorf("expected scoreboard to be empty after map change")
	}
	if board.GameStart() != (time.Time{}) {
...
func TestCVarStorage(t *testing.T) {
	...
	if got := board.CVar("mp_timelimit"); got != "30" {
...
func TestPointCapture(t *testing.T) {
	...
	if p, _ := board.Player("STEAM:1"); p.Caps != 1 {
		t.Errorf("capper A caps = %d, want 1", p.Caps)
	}
	if p, _ := board.Player("STEAM:2"); p.Caps != 1 {
...
func TestDisconnectedRemovesPlayer(t *testing.T) {
	...
	if _, ok := board.Player("UID:11"); !ok {
...
func TestHealed(t *testing.T) {
	...
	if p, _ := board.Player("1"); p.Heals != 75 {
...
func TestRoundStartSetsGameStart(t *testing.T) {
	...
	board.Apply(logparse.Event{Type: logparse.EventRoundStart, Timestamp: start})

	if got := board.GameStart(); !got.Equal(start) {
